...
export type ScheduleDoc = { day: Day; version: number; timezone?: string; items: ScheduledItem[] };
...
export async function getDaySchedule(params: { channel: string; week: string; day: Day }): Promise<ScheduleDoc> {
  ...
  return { day, version: doc.version ?? 0, timezone: doc.timezone, items: doc.items ?? [] };
}

export async function putDaySchedule(params: { channel: string; week: string; day: Day; items: ScheduledItem[]; version: number }): Promise<ScheduleDoc> {
  const { channel, week, day, items, version } = params;
  const res = await fetch(`${CONFIG.API_BASE_URL}/schedule/${encodeURIComponent(channel)}/${encodeURIComponent(week)}/${day}`, {
    method: 'PUT',
    headers: {
  ...
  const doc = await res.json();
  return { day, version: doc.version ?? version + 1, timezone: doc.timezone, items: doc.items ?? items };
}
